...
3. **Known gaps are not deductions.** A gap listed in `docs/design-system.md` → Known gaps or `docs/component-gaps.md` that is reported and left alone is correct behaviour. Deduct when a gap is hidden, worked around silently, or filled with an invented component or token.
...
- **4:** Raw values in several places: a loose `12px` gap, an `rgba()` scrim, a duration typed as `300ms`. Some screens hand-roll what a component already does, for example a verdict drawn as a styled `span` instead of `VerdictChip`, or a second primary drawn as a custom pill. A prop is used that no story shows. Scale groups are misread, so `Icon/400` is used where `Radius/400` was meant.
...
- **9:** Five of five sampled values trace to a semantic token, and the trace was done in the rendered DOM, not in source. Every inline pattern is logged in `component-gaps.md`, and anything used twice has been promoted or flagged. New tokens (`Stroke/Bold`, `motion.duration.spin`) were added through `tokens.json` with `design-system.md` updated in the same pass. Reduced motion is the `$extensions` mode, not a parallel set of values. No component was invented to fill a Known gap.
...
**How to check:** walk SPEC.md → Verification paths 1, 7 and 8 back to back. Compare the section round, the review round and the exam-eve repeat, then the three summaries side by side.

- **4:** The voice turn and the typed turn look like two apps. The switch to typing ("Type instead" or "Use my voice instead") sits in a different place on each. The summaries each invent their own layout for counts. Knowie shows up at arbitrary sizes. One screen uses capitals or a tone that no other screen uses.
...
- **9:** The loop is recognisably the same in all three rounds, and each round is framed for what it can honestly claim. The section summary says completion, not mastery. The review headline states the count and the gap, or "still fresh" on the same day. The exam-eve summary leads with the work left. The three outcome categories are named the same way on every screen. The typed and voice turns share the ladder, the verdicts and the position of the mode switch. The leave confirm, the mic prompt and the tray use one sheet language. A tester walking all three rounds never has to relearn anything.
...
**How to check:** in device mode at 390×844, press every control and watch it. Step through every `VoiceInput` state. Measure the gaps between Knowie, the bubble, the ring and the escapes. Compare entrance and exit timings.
...
- **6:** Press is geometry and the lip is intact. The states exist and are distinguishable. But the transitions between them are abrupt: the transcript appears instead of growing out of the ring, or exits run at the same speed as entrances. Or one state was clearly finished last: say-it-back looks like idle with its label changed.
- **9:** Every rule in `design-system.md` → How things behave holds on inspection. Press sinks by the lip depth at `motion.duration.instant`. Exits run about a third faster than entrances. One `accent/brand/bold` line carries idle through judging and grows into the transcript card. The slow beat at 4 seconds is a visible change (dashes at `spin-slow`), not a relabel. The error is grey with `refresh-cw-01`, and "Tap to send again" re-runs judging on the same take. No check mark appears anywhere in the control. The glow is a shape, not an effect. Spacing between Knowie, the bubble and the control holds across all sixteen voice-turn states. Every one of these was watched in the browser, not read in CSS.
...
**How to check:** tick off every Must row in `voice-ux.md` → States to design, and every state listed under Voice turn in SPEC.md, in the running app. Walk paths 2 to 6 (discard and unclear, I don't know, typed, denied, leave and resume).

- **4:** One or more Must states is missing or dead-ends. Permission denied leaves the student stuck. The wait has no status. Discard sends the take anyway. The student is trapped somewhere: no route to typing, or X leaves without a confirm. A hinted pass is counted as unaided on a summary, or a summary shows a score that cannot be recounted from the rows.
...
- **9:** All six `voice-ux.md` principles are visible on screen.
  1. Status is unmistakable at each of idle → listening → transcribing → transcript → judging → result, and stays so with motion reduced.
  ...
  Outcomes stay honest: hinted never merges into unaided, every summary number recounts from `sessionStorage` rows, and practice retries change no rows. Overconfidence is named in the review copy and underconfidence gets evidence. Walked by tapping, per SPEC.md → Verification §3 and §5.
...
- **9:** Measured contrast for every text/background pair on every screen. Targets inspected at 44 or more everywhere, including `ButtonIcon` S and M wrappers and the five confidence radios. Every state difference is carried by shape, icon, label or motion as well as colour, and the error being grey rather than red is deliberate. `VoiceInput`'s label is a live region that names the state on its own with motion reduced. Sub-minimum type is `aria-hidden` with the meaning moved to `aria-valuetext`. The a11y addon's violations list was read and is empty or triaged. The missing focus state is reported as the known WCAG 2.2 gap, not hidden.

## 6. Structure (Low)

**What it scores:** whether the layout holds together and the thing renders.
...
- **6:** Everything builds and renders, and the safe areas hold. But a 40% string expansion clips a button label or overlaps the bubble and the ring, or a summary with twelve rows pushes the primary action off-screen instead of scrolling `middleContent`.
