...
- **Every value comes from `tokens.json`** — colour, space, radius, duration, icon size, type step. Read the group before the number: the scales differ, so `Radius/400` and `Icon/400` are different sizes. If the value isn't there, see Never.
...
- **When something is missing** from `tokens.json` or the component set: say what's missing and what you'd call it, then stop.
...
**Design system**
- `tokens.json` — every time you need a number, colour, duration or type step. Never edit without updating `docs/design-system.md` in the same pass.
